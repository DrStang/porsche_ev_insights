    ...
    <div className={`fixed inset-0 ${darkMode ? 'bg-black/70' : 'bg-black/50'} backdrop-blur-sm z-50 flex items-center justify-center p-4`}>
      <div className={`rounded-2xl border max-w-md w-full p-5 ${darkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200'}`}>
        <div className="flex items-center justify-between mb-5">
          <h2 className={`text-lg font-bold ${darkMode ? 'text-white' : 'text-zinc-900'}`}>{t('upload.title')}</h2>
        ...
        <div className="space-y-3">
          ...
          <button onClick={processUploadedFiles} disabled={!uploadStatus.start} className={`w-full py-2.5 rounded-xl bg-sky-500 text-white font-bold ${!uploadStatus.start ? 'opacity-50' : ''}`}>{t('upload.processButton')}</button>
        </div>
      </div>
    </div>
  );
